...
function Tween:Update(elapsedTime)
	 self.timePassed = self.timePassed + (elapsedTime or love.timer.getDelta())
...
function Tween:Reverse()
	log.trace("TWEEN REVERSE")
	self.timePassed = 0
	self.isFinished = false
	self.startValue = self.current
	self.distance = -self.distance
end

-- Added this for staying in MoveState
function Tween:Reset()
	self.current = self.startValue
	self.timePassed = 0
	self.isFinished = false
end
